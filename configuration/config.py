...
class Configuration(metaclass=Singleton):
    ...
    def get_tables_sql_query(self):
        with open(os.path.abspath(os.path.join(os.path.dirname(__file__),'..','assets/sql_scripts/tables.sql'))) as query:
            return query.read()

    def get_columns_sql_query(self):
        with open(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'assets/sql_scripts/columns.sql'))) as query:
            return query.read()

    def get_FK_sql_query(self):
        with open(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'assets/sql_scripts/foreign_keys.sql'))) as query:
            return query.read()

    def get_PK_sql_query(self):
        with open(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'assets/sql_scripts/primary_keys.sql'))) as query:
            return query.read()
